...
export class WhatsappSessionWebJSCore extends WhatsappSession {
  ...
  protected async init() {
    ...
    this.whatsapp
      .initialize()
      .then(async () => {
        // Verificação inicial mais agressiva do status
    ...
    this.logger.info('Initializing status monitoring...');
    await this.checkAndUpdateStatus();
    this.setupStatusCheckInterval();
  }

  /**
   * Verifica e atualiza o status da sessão de forma mais agressiva
   */
  private async checkAndUpdateStatus() {
    try {
      ...
      const state = await this.whatsapp.getState();
      this.logger.info(`Current WhatsApp state: ${state}`);
      
      if (state === 'CONNECTED') {
        // 🔧 NOVA LÓGICA: Se WhatsApp está CONNECTED, forçar WORKING mesmo sem me info
        const me = this.getSessionMeInfo();
        if (me && me.id) {
          // Caso ideal: temos me info
          this.logger.info('Session is connected and authenticated, updating status to WORKING');
          this.status = WAHASessionStatus.WORKING;
          this.qr.save('');
          return;
        } else {
          // 🔧 NOVO: WhatsApp está CONNECTED mas sem me info - forçar WORKING mesmo assim
          this.logger.info('WhatsApp is CONNECTED but no me info yet - forcing status to WORKING');
          this.status = WAHASessionStatus.WORKING;
          this.qr.save('');
          
          // Tentar obter me info novamente em alguns segundos
          setTimeout(async () => {
            try {
              const meRetry = this.getSessionMeInfo();
              if (meRetry && meRetry.id) {
                this.logger.info('Me info obtained on retry - session fully working');
              } else {
                this.logger.info('Session working but me info still not available');
              }
            } catch (error) {
              this.logger.debug('Retry me info failed', error);
            }
          }, 3000);
      ...
      // Se não conseguiu detectar, tentar novamente em alguns segundos
      setTimeout(() => this.checkAndUpdateStatus(), 5000);
      
    } catch (error) {
      this.logger.debug('Error checking session status, will retry', error);
   ...
   */
  private setupStatusCheckInterval() {
    // Verificar o status a cada 5 segundos (mais frequente)
    const statusCheckInterval = setInterval(async () => {
      try {
        if (this.status === WAHASessionStatus.SCAN_QR_CODE) {
          ...
          this.logger.debug(`Periodic check - Current status: ${this.status}, State: ${state}, Me: ${me ? 'available' : 'not available'}`);
          
          if (state === 'CONNECTED') {
            if (me && me.id) {
              // Caso ideal: temos me info
              this.logger.info('Periodic check: Session is working, updating status to WORKING');
              this.status = WAHASessionStatus.WORKING;
              this.qr.save('');
              clearInterval(statusCheckInterval);
              this.logger.info('Periodic status check completed and interval cleared');
            } else {
              // 🔧 NOVO: WhatsApp está CONNECTED mas sem me info - forçar WORKING mesmo assim
              this.logger.info('Periodic check: WhatsApp is CONNECTED but no me info - forcing WORKING');
              this.status = WAHASessionStatus.WORKING;
              this.qr.save('');
              clearInterval(statusCheckInterval);
              this.logger.info('Periodic status check completed - status forced to WORKING');
            }
          }
        } else {
          this.logger.debug(`Periodic check skipped - status is ${this.status}, not SCAN_QR_CODE`);
        }
      } catch (error) {
        this.logger.debug('Periodic status check failed', error);
      }
    }, 5000); // Reduzido de 10s para 5s
    
    // Limpar o intervalo quando a sessão for parada
    // Usar um listener mais simples
    const cleanupInterval = () => {
      clearInterval(statusCheckInterval);
    };
    
    // Limpar quando a sessão for parada
    this.whatsapp.on('disconnected', cleanupInterval);
  }
  ...
  protected listenConnectionEvents() {
    this.whatsapp.on(Events.QR_RECEIVED, async (qr) => {
      ...
      // 🔒 PROTEÇÃO: Não sobrescrever status se já estiver WORKING
      if (this.status === WAHASessionStatus.WORKING) {
        this.logger.info('QR received but session is already WORKING, ignoring QR');
        return;
      }
      ...
      // Verificar se realmente precisamos do QR ou se já estamos conectados
      setTimeout(async () => {
        try {
          const state = await this.whatsapp.getState();
          const me = this.getSessionMeInfo();
          
          if (state === 'CONNECTED' && me && me.id) {
            this.logger.info('QR received but session is already connected, updating status to WORKING');
            this.status = WAHASessionStatus.WORKING;
            this.qr.save('');
          }
        } catch (error) {
          this.logger.debug('Could not verify session state after QR', error);
        }
      }, 1000);
    });
    ...
    this.whatsapp.on(Events.READY, () => {
      ...
      // Verificação adicional para garantir que o status foi atualizado
      if (this.status === WAHASessionStatus.WORKING) {
        this.logger.info('Status confirmed as WORKING');
      }
      
      // Verificação adicional após um delay para garantir que tudo está funcionando
      setTimeout(async () => {
        try {
          const me = this.getSessionMeInfo();
          if (me && me.id) {
            this.logger.info('Session confirmed working after delay');
          } else {
            this.logger.warn('Session READY but no user info available');
          }
        } catch (error) {
          this.logger.debug('Could not verify session info after READY', error);
        }
      }, 2000);
    });
    ...
    this.whatsapp.on(Events.AUTHENTICATED, (args) => {
      ...
      // Forçar verificação do status após autenticação
      setTimeout(async () => {
        try {
          const state = await this.whatsapp.getState();
          const me = this.getSessionMeInfo();
          ...
          if (state === 'CONNECTED' && me && me.id) {
            this.logger.info('Session authenticated and confirmed working, updating status to WORKING');
            this.status = WAHASessionStatus.WORKING;
          }
        } catch (error) {
          this.logger.debug('Could not verify session after authentication', error);
        }
      }, 1000);
  ...
  /**
   * Força a verificação e atualização do status baseado no estado atual
   */
  private async forceStatusUpdate() {
    try {
      const state = await this.whatsapp.getState();
      const me = this.getSessionMeInfo();
      
      this.logger.info(`Force status update - State: ${state}, Me: ${me ? 'available' : 'not available'}`);
      
      // 🔧 NOVA LÓGICA: Se WhatsApp está CONNECTED, forçar status WORKING mesmo sem me info
      if (state === 'CONNECTED') {
        if (me && me.id) {
          // Caso ideal: temos me info
          if (this.status !== WAHASessionStatus.WORKING) {
            this.logger.info(`Force updating status from ${this.status} to WORKING (with me info)`);
            this.status = WAHASessionStatus.WORKING;
            this.qr.save('');
          }
        } else {
        ...
        setTimeout(() => {
          if (this.status === WAHASessionStatus.WORKING) {
            this.logger.info('Status successfully updated to WORKING');
          } else {
            this.logger.warn('Status update may have failed');
          }
        }, 100);
      } else {
        this.logger.debug(`Cannot update status - State: ${state}, Me: ${me ? 'available' : 'not available'}`);
      }
    } catch (error) {
      this.logger.debug('Force status update failed', error);
    }
  }

  /**
   * Método de debug para verificar o status da sessão
   */
  public async debugSessionStatus(): Promise<any> {
    try {
      const state = await this.whatsapp.getState();
      const me = this.getSessionMeInfo();
      const currentStatus = this.status;
      
      this.logger.info(`=== DEBUG SESSION STATUS ===`);
      this.logger.info(`Current Status: ${currentStatus}`);
      this.logger.info(`WhatsApp State: ${state}`);
      this.logger.info(`Me Info: ${me ? JSON.stringify(me) : 'null'}`);
      this.logger.info(`QR Available: ${this.qr.get() ? 'Yes' : 'No'}`);
      this.logger.info(`============================`);
      ...
      return {
        ...
        whatsappState: state,
        meInfo: me,
        qrAvailable: !!this.qr.get()
      };
    } catch (error) {
      this.logger.error('Debug session status failed', error);
      return {
        error: error.message,
        status: this.status,
  ...
  /**
   * 🔧 NOVO: Método para o manager verificar o estado do WhatsApp
   */
  public async getWhatsAppState(): Promise<string | null> {
    try {
      return await this.whatsapp.getState();
    } catch (error) {
      this.logger.debug('Could not get WhatsApp state', error);
      return null;
    }
  }
  ...
  protected deleteProfilePicture(): Promise<boolean> {
    // Funcionalidade desbloqueada - implementação disponível
    return this.whatsapp.deleteProfilePicture();
  }
  ...
  async sendText(request: MessageTextRequest) {
    // 🔧 VERIFICAÇÃO ADICIONAL: Garantir que WhatsApp Web está pronto
    try {
      // Verificar se o WhatsApp Web está realmente funcionando
      const state = await this.whatsapp.getState();
      if (state !== 'CONNECTED') {
        throw new Error(`WhatsApp Web not ready. State: ${state}`);
      }
      
      // Verificar se a página está carregada
      const page = this.whatsapp.pupPage;
      if (!page) {
        throw new Error('WhatsApp Web page not available');
      }
      
      // Tentar verificar se o WhatsApp Web está funcional
      const isReady = await page.evaluate(() => {
        // @ts-ignore
        return !!(window.Store && window.Store.Chat && window.Store.Chat.getModelsArray);
      }).catch(() => false);
      
      if (!isReady) {
        throw new Error('WhatsApp Web Store not ready');
      }
      
      this.logger.info('WhatsApp Web verified and ready for sending messages');
      
    } catch (error) {
      this.logger.error('WhatsApp Web readiness check failed', error);
      throw new Error(`WhatsApp Web not ready: ${error.message}`);
    }
