  ...
  implements BeforeApplicationShutdown, OnApplicationBootstrap
{
   ...
   * 🔧 MELHORADO: Verifica tanto o status interno quanto o estado do WhatsApp
   */
  async waitUntilStatus(
    ...
    // 🔧 NOVA LÓGICA: Verificação inteligente de status
    const valid = await waitUntil(
      async () => {
        // 1. Verificar se o status interno está correto
        if (expected.includes(session.status)) {
          return true;
        }
        
        // 2. 🔧 SE status for SCAN_QR_CODE mas WhatsApp estiver CONNECTED, considerar WORKING
        if (session.status === WAHASessionStatus.SCAN_QR_CODE && expected.includes(WAHASessionStatus.WORKING)) {
          try {
            // Verificar se o método getWhatsAppState existe (WEBJS engine)
            if (typeof (session as any).getWhatsAppState === 'function') {
              const whatsappState = await (session as any).getWhatsAppState();
                             if (whatsappState === 'CONNECTED') {
                 this.log.logger.info(`Session ${sessionName} is CONNECTED but status is SCAN_QR_CODE, forcing WORKING`);
                 session.updateStatus(WAHASessionStatus.WORKING);
                 return true;
               }
            }
          } catch (error) {
            this.log.logger.debug(`Could not check WhatsApp state for session ${sessionName}`, error);
          }
        }
