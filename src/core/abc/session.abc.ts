...
export abstract class WhatsappSession {
  ...
  /**
   * 🔧 NOVO: Método público para atualizar o status da sessão
   * Permite que outras classes (como SessionManagerCore) atualizem o status
   */
  public updateStatus(newStatus: WAHASessionStatus): void {
  ...
  /**
   * Force refresh session status - useful when status detection fails
   */
  public async forceRefreshStatus(): Promise<void> {
    try {
      // Verificar se a sessão está realmente funcionando
      const me = this.getSessionMeInfo();
      if (me && me.id) {
        this.logger.info('Session is working, forcing status update to WORKING');
        this.status = WAHASessionStatus.WORKING;
        return;
      }
      
      // Se não conseguiu detectar pelo me, tentar outras abordagens
      this.logger.info('Could not detect session info, trying alternative methods');
      
      // Aguardar um pouco e tentar novamente
      await sleep(1000);
      const meRetry = this.getSessionMeInfo();
      if (meRetry && meRetry.id) {
        this.logger.info('Session detected on retry, updating status to WORKING');
        this.status = WAHASessionStatus.WORKING;
        return;
      }
      
      this.logger.warn('Session status refresh failed - session may not be properly authenticated');
      
    } catch (error) {
      this.logger.debug('Could not force refresh status', error);
    }
  }

  /**
   * Debug session status - abstract method to be implemented by engines
   */
  public async debugSessionStatus(): Promise<any> {
    // Implementação padrão que pode ser sobrescrita pelos engines
    return {
      status: this.status,
      name: this.name,
      engine: this.engine,
      error: 'Debug method not implemented for this engine type'
    };
  }
}
