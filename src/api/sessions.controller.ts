...
class SessionsController {
  ...
  @Get(':session/debug')
  @SessionApiParam
  @ApiOperation({ summary: 'Debug session status and state' })
  async debugSession(@SessionParam session: WhatsappSession): Promise<any> {
    // Verificar se o método debugSessionStatus existe
    if (typeof session.debugSessionStatus === 'function') {
      return await session.debugSessionStatus();
    } else {
      return {
        error: 'Debug method not available for this session type',
  ...
  @Post(':session/refresh-status')
  @SessionApiParam
  @ApiOperation({
    summary: 'Force refresh session status',
    description: 'Force refresh the session status when detection fails',
  })
  async refreshStatus(@Param('session') name: string): Promise<SessionDTO> {
    await this.withLock(name, async () => {
      const exists = await this.manager.exists(name);
      if (!exists) {
        throw new NotFoundException('Session not found');
      ...
      await session.forceRefreshStatus();
    });
    return await this.manager.getSessionInfo(name);
  }
  ...
  @SessionApiParam
  @ApiOperation({
    summary: 'Force session reconnection',
    description: 'Force session reconnection when status detection fails completely',
  })
  async forceReconnect(@Param('session') name: string): Promise<SessionDTO> {
    await this.withLock(name, async () => {
      const exists = await this.manager.exists(name);
      if (!exists) {
        throw new NotFoundException('Session not found');
